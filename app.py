...
try:  # address-specific zoning / land-use answers (Burbank Planning); opt-in via ZONING_ROUTE_ENABLED
...
ROUTER_SYSTEM_MESSAGE = (
    ...
    "allowed in [zone]', 'what can I do with the property at [address]', and short follow-ups "
    "that supply a zoning designation (e.g. 'zoning is C-3', 'it's C-3', or just 'C-3', 'R-1', "
    "'MDC-3') answering an earlier land-use question. Examples: 'can I open a medical office at "
    "2019 W Magnolia', 'is a restaurant allowed in C-3', 'zoning is C-3'. This is about a "
    "SPECIFIC property or zone, NOT a general question about what the code says (that is website).\n"
...
EVENTS_ENABLED = (bool(events_feed)
                  and os.environ.get("EVENTS_ROUTE_ENABLED", "0") != "0"
                  and events_feed.available())
...
async def try_zoning_answer(request_body, domain):
    """Address-specific zoning / land-use questions -> Planning-style two-turn answer (ask for the
    zoning designation, then answer). Returns the answer string, or None to fall through to RAG."""
    ...
    try:
        client = await init_openai_client()
        history = _recent_history(messages)
        logging.info("[ZONING] handling: %s", user_query)
        return await zoning.answer_zoning_query(
            user_query, client, app_settings.azure_openai.model, history=history)
    except Exception:
        logging.exception("zoning route failed; falling back to RAG")
        return None
...
async def complete_chat_request(request_body, request_headers):
    if app_settings.base_settings.use_promptflow:
        ...
        if zoning_answer is not None:
            return permit_non_streaming_response(zoning_answer, history_metadata)
...
async def stream_chat_request(request_body, request_headers):
    ...
    zoning_answer = await try_zoning_answer(request_body, domain)
    if zoning_answer is not None:
        return permit_stream_response(zoning_answer, history_metadata)
    website = await try_website_answer(request_body, domain)   # website/codes -> code pipeline
    if website is not None:
